...
public class SlackClient {
  ...
  private final WebHookToken webHookToken;

  public SlackClient() {
    String channelToken = SystemProperties.SLACK_CHANNEL_TOKEN;
    ...
    webHookToken =
        WebHookToken.builder().partT(tokens[0]).partB(tokens[1]).partX(tokens[2]).build();
  }
  ...
  public void postMessage(Attachment... attachments) {
    MessageRequest messageRequest =
        MessageRequest.builder()
            ...
            .text(
                "Autobot Test Report "
                    + SlackMarkdown.LINK.decorate(url(SystemProperties.JOB_URL), " [Job Link]"))
  ...
  public Attachment buildGroupAttachment(boolean pass, Field... fields) {
    Attachment.Builder attachment =
        Attachment.builder().addFields(fields).title(Title.of("Test Status"));
