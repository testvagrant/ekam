...
import com.testvagrant.ekam.devicemanager.devicefinder.LambdaTestDeviceFinder;
...
public class LambdaTestDriver {
  ...
  public LambdaTestDriver(MobileConfigParser mobileConfigParser) {
    this.mobileConfigParser = mobileConfigParser;
    this.cloudConfig = new ConfigLoader().loadConfig("lambdatest");
  ...
  public Triple<URL, DesiredCapabilities, TargetDetails> buildRemoteMobileConfig() {
    ...
    Map<String, Object> lambdaTestCaps =
        new CapabilityMapper().mapToLambdaTestCaps(appUrl, target);
    DesiredCapabilities updatedCapabilities =
        desiredCapabilities.merge(new DesiredCapabilities(lambdaTestCaps));
  ...
  private TargetDetails findTarget() {
    return new LambdaTestDeviceFinder(
